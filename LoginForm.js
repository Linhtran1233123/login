...
export default function LoginForm(){
    ...
    const validationStrict = Yup.object().shape({
        ...
        // password: Yup.string().required().matches(/.{8,}/,{message: "unValidLength ",excludeEmptyString: true}).matches(/(?=.*?[0-9])/,{message: "missingNumber"}).matches(/(?=.*?[A-Z])/,{message: "missingUpper"}).matches(/(?=.*?[a-z])/,{message: "missingLower"}).matches(/(?=.*?[#?!@$%^&*-])/,{message: "missingSymbol"}),
        // passwordConfirm:Yup.string().required().when("password",(password,schema)=>{
        //     return password ? schema.oneOf([Yup.ref("password")],"Passwords do not match "):schema
        // })
        passwordConfirm: Yup.string().required().oneOf([pwValue],"Passwords do not match ")
    })
    useEffect(() => {
                ...
                <div className="form-container sign-up-container">
                    ...
                    >
                        ...
                        <Form id="register-form">
                            ...
                            {/*<Field type="password" id=" passwordConfirm" placeholder="Password confirm" name="passwordConfirm"/>*/}
